...
class AllProjects extends Component {
  ...
  loadProjectsFromServer(){
    var self = this;
    axios.get("http://localhost:3001/project/getallprojects")
    ...
    if(this.state.data != null){
      projectList = this.state.data.map(project => {
        return(
          ...
          max_budget = {project.max_budget} min_budget = {project.min_budget} employer_id = {project.userId} employer_name={project.name}   />
        )
      })
    }
    return (

  ...
  render() {
    ...
    if(this.state.data != null){
      projectList = this.state.data.map(project => {
        return(
          <UserBidProject key = {project.id} employer_id = {project.employer_id}  project_name = {project.title} employer_name={project.freelancer_name} avg_bid={project.avg_days}
          user_bid={project.bid_price} project_id = {project.id} assigned_to = {project.assigned_to}    />
        )
      })
    }
    return (

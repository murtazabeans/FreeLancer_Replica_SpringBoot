...
class ProjectView extends Component {
  ...
  loadProjectDetailsFromServer(project_id){
    var self = this;
    axios.post("http://localhost:3001/project/get_project_detail", {project_id: project_id})
    .then(function (response) {
      if(response.data.rows != null){
        let user_detail = response.data.rows[0];
        console.log(response);
        if(response.data.rows[0].days == null){
          response.data.rows[0]["days"] = "0";
        }
